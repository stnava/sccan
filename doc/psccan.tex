...
%this paper.} 
% Paramveer: We dont need this now.

Recently, sparse (or penalized) canonical covariance
analyses~\cite{parkhomenko,witten,lykou} (SCCovA) have been proposed
as an approximation to SCCA specifically for the high dimensional ($p\gg n$)
%%Paramveer: above it should be "approximation to SCCA" and not CCA.  I corrected it
 setting where $n$ is the number of observations (subjects in our
...
pseudoinverse when needed.  In addition, the function $(x)_+$ is equal to $x$ if $x \geq 0$ and $0$ if $x <0$ and 
 \begin{equation}
