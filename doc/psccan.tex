...
in the brain).  SCCovA methods approximate within-view covariance
matrices as the identity and thus analyze cross-covariance structure, not
...
However, none of the previous work handle confounding variables-- a
...
contribution of this paper i.e. a way of ``partialing'' out nuisance
variables in the SCCA setting.  We also provide a power iteration
based algorithm to efficiently compute the directions of maximum
correlation. Our approach incorporates the normalization constraints
required for correlation (versus covariance) analysis and is called
