...
\label{partial-mic}
\begin{algorithmic}[1]
\STATE Apply the whitening transformation to {\X}, {\Y}, {\Z} to get  ${\X_w}$, ${\Y_w}$, ${\Z_w}$. 
...
\STATE Compute  $\bs \phi_Y^{k+1}= {\Y_w}^T {\X_w} \bs \phi_X^{k+1} -  {\Y_w}^T  {\Z_w} {\Z_w}^T {\X_w} \bs \phi_X^{k+1}$
