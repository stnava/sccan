...
\includegraphics[width=0.9\textwidth]{./figs/sparseness.jpg}
...
\input{src/behav_network_f.txt}

\input{src/exec_network_f.txt}

\input{src/lang_network_f.txt}

\input{src/mem_network_f.txt}

\input{src/vs_network_f.txt}
...
ff <- \hlfunctioncall{sparseDecom2}(inmatrix = \hlfunctioncall{list}(trainmat, cogmat[permutesubs, nw]), 
    inmask = \hlfunctioncall{c}(maskimg, NA), sparseness = \hlfunctioncall{c}(sigfrac, -1), 
    nvecs = 1, its = 15 )
myproj1a <- trainmat %*% \hlfunctioncall{t}(\hlfunctioncall{imageListToMatrix}(ff$eig1, maskimg))
myproj1b <- cogmat[permutesubs, nw] %*% \hlfunctioncall{as.matrix}(ff$eig2)
