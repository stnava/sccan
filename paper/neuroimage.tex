...
\usepackage{geometry,tikz}                % See geometry.pdf to learn the layout options. There are lots.
...
\label{algo1}
\KwIn { {\bf X}, {$\bf M$}, $\lambda$, $\gamma$. }
Normalize the data matrix {\bf X} by  mean centering it\;
\For{i=1 to k}{
	Initialize the $i^{th}$ eigestor $\bf v_{i} \gets M_i$ based on the corresponding prior\;
	Compute {$\bf u_i$} i.e. the $i^{th}$ row of the $\bf U$ matrix by minimizing the reconstruction error as ${\bf u_i^*}= \argmin_{u_i} \|{\bf x_i} - {\bf u_i}\otimes {\bf v_i} \|_2^2$\;
}
\For {i=1 to k}{
	Compute the partial {\bf X} matrix: ${\bf X}_{\backslash i}= {\bf X}- \sum_{j=1, j\neq i}^{k}  {\bf u_j}\otimes {\bf v_j}$\;
	${\bf v_i}$ $\gets$ {\bf SPCG}$({\bf X}_{\backslash i}, {\bf M_i}, \lambda,\gamma, {\bf v_i}, numIterations)$ //Perform Sparse Partial Conjugate Gradient\;
	Normalize the eigestor ${\bf v_i}$ $\gets$ ${\bf v_i}/\|{\bf v_i}\|$\;
	${\bf v_i}$ $\gets$  $S(v,\gamma)$ //Soft-Max and Cluster Thresholding\;
	}
\KwOut {$\{{\bf v}\}_{i=1}^{k}$}
\end{algorithm}
...
\label{algo2}
\KwIn  {${\bf A}, {q}, \delta,\mu, {w}$, n.}
$j \gets 0$ //Iteration Counter\;
${ p} \gets (1-\delta) \cdot {\bf A}w + \delta \cdot qq^{\top}w$\;
Initialize  Rayleigh Coefficient  $R$=0, $R_{old}=-1$\;
g $\gets$ w\;
bestW $\gets$ w\;
...
\While {$R >R_{old}$ $\&\&$ j $<$ n}{
	$\mu=1$\;
	t $\gets$ $qq^{\top}w$\;
	s $\gets$  ${\bf A}^{\top}\times ({\bf A}\times w)$\;
         s $\gets$  $(1-\delta)\cdot$ s + $\delta\cdot$ t\;
         $\mu \gets \frac{s\cdot s}{g\cdot g}$\;
         g $\gets$ s\;
         $w \gets w + s\times \mu \times$ gradStep //gradStep chosen as $0.1$\;
         Sparsify($w$)\;
         $w \gets \frac{w}{\|w\|}$\;
         ${p} \gets (1-\delta) \cdot {\bf A}w + \delta\cdot qq^{\top}w$\;
        $R_{old}=R$\;
	Compute $R \gets \frac{p\cdot p}{w\cdot w}$\;
	$j \gets j+1$\;
	\If {$R > R_{old}$ $\|$ j==1}{
	bestW $\gets$ w\;
	}
...
%We used two cortical label (probabilistic ROIs) definitions for our experiments 1). Non-Rigid Image Registration Evaluation Project (NIREP),  (\url{http://www.nirep.org/}), 32 labels in total and  2). LONI Probabilistic Brain Atlas (LPBA40)~\citep{lpba}, 55 labels in total.
